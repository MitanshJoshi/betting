...
import { FormControl, MDSelect, MenuItem, Select } from "@mui/material";
...
const Addcontests = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch(
                `${BASE_URL}/api/contest/update?contestId=${_id}`,
                {
                    method: "PUT",
                    headers: {
    ...
    const contestType = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(
                `${BASE_URL}/api/contestType/display`,
                {
                    method: "GET",
                    headers: {
                        Authorization: token,
    ...
    const fetchContestDetails = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(
                `${BASE_URL}/api/contest/displayDetails?contestId=${_id}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: token,
            ...
            if (responseData.success) {
                setLeague_id(data.leagueData[0]._id);
                setMatchId(data.match_id);
                setContestId(data.contest_type_id);
                setPrice(data.price_pool);
                setProfit(data.profit);
                setEntry(data.entry_fees);
                setTotal(data.total_participant);
                setMax(data.max_team_per_user);
                fetchMatches(data.leagueData[0]._id);
            }
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        ...
        //                                 </MDBox>
        //                                 <MDBox mb={2}>
        //                                     <label htmlFor="max-team-per-user">
        //                                         Max Team per User
        //                                     </label>
        //                                     <MDInput
        //                                         type="number"

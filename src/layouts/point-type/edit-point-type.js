...
const AddPointType = () => {
    ...
    const fetchPointTypeById = async () => {
        try {
            const token = localStorage.getItem("token");
            console.log(token);
            ...
            const responseData = await response.json();
            console.log(responseData);
            const array = responseData.data[0].point_for.map((x) => x._id);
            setSelectedIds(array);
            setPointTypeName(responseData.data[0].point_type_name);
        } catch (error) {
            console.error("Error fetching data from the backend", error);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log(token);
            const response = await fetch(
                `${BASE_URL}/api/pointtype/update/${id}`,
                {
                    method: "PUT",
                    headers: {
                        Authorization: token,

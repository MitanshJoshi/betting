...
const PointType = () => {
    ...
    const rows = data?.map((item) => ({
        name: <Author name={item?.point_type_name} />,
        delete: (
            <MDButton
                onClick={() => {
        ...
        edit: (
            <MDButton
                color={"info"}
                size={"small"}
                href={`/points-type/edit-points-type/${item?._id}`}
            >
                <MDTypography
                    component="a"
                    variant="caption"
    ...
    const handleDelete = async () => {
        try {
            ...
            console.log(token);
            const response = await fetch(
                `${BASE_URL}/api/pointtype/delete/${deleteId}`,
                {
                    method: "DELETE",
                    headers: {
                        Authorization: token,
                    ...
                    <Grid item xs={12}>
                        <Card>
                            ...
                            >
                                ...
                                <MDButton
                                    size={"small"}
                                    href={"/points-type/add-points-type"}
                                >
                                    <MDTypography
                                        variant="button"
                                        color="black"

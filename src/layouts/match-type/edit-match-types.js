...
const AddMatchType = () => {
    ...
    const fetchMatchTypeById = async () => {
        try {
            const token = localStorage.getItem("token");
            console.log(token);
            const response = await fetch(`${BASE_URL}/api/matchType/${id}`, {
                method: "GET",
                headers: {
                    Authorization: token,
                },
            ...
            const array = responseData.data.pointType.map((x) => x._id);
            setSelectedIds(array);
            setMatchTypeName(responseData.data.name);
        } catch (error) {
